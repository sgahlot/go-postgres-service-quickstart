...
func TestRetrieveAllFruits(t *testing.T) {
	...
	response, err := invokeApiAndVerifyResponse(t, router, "GET", "/api/v1/fruits?name="+db.ALL_ROWS, http.StatusOK)
	fruitResponse := response.(db.FruitResponse)
...
func TestInsertFruit(t *testing.T) {
	...
	fruitRequest := db.FruitRequest{Description: "Full of fibre adn Vitamin C", Name: "Pear"}
